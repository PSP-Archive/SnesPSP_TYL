...
*/
  ...
  uint32 black = BLACK | (BLACK << 16);
	...
	{
	  GFX.r2131 = 0x5f;
	  GFX.r212d = (FillRAM [0x212c] ^
		       FillRAM [0x212d]) & 15;
	  GFX.r212c &= ~GFX.r212d;
	  GFX.r2130 |= 2;
	}

      if (!os9x_easy && !PPU.ForcedBlanking && ADD_OR_SUB_ON_ANYTHING &&
	  (GFX.r2130 & 0x30) != 0x30 &&
	  ...
	  if (pClip->Count [5])
	    {
		  ...
		  for (uint32 c = 0; c < pClip->Count [5]; c++)
			...
			{
			  memset (GFX.SubZBuffer + y * GFX.ZPitch + pClip->Left [c][5] * x2,1, 
			  ...
			  if (IPPU.Clip [0].Count [5])
			    {
			      ...
			      while (p < q) *p++ = (uint16) GFX.FixedColour;
			    }
			}
		    }
		}
