...
void DrawBGMode7BackgroundNew (uint8 *Screen, int bg, int pitch)
{
    RENDER_BACKGROUND_MODE7NEW_PHASE2 (uint8, (uint8) (b & GFX.Mode7Mask),pitch)
}

void DrawBGMode7BackgroundPrioNew (uint8 *Screen, int bg,int prio,int pitch)
{
	if ((GFX.Mode7PriorityMask >> 7)!=prio) { return; } else {
    	DrawBGMode7BackgroundNew (Screen, bg,pitch);
 	}
}

void DrawBGMode7Background16New (uint16 *Screen, int bg,int pitch)
{
    // /*GFX.ScreenColors [b & GFX.Mode7Mask]*/
    RENDER_BACKGROUND_MODE7NEW_PHASE2 (uint16, (GFX.ScreenColors [b]),pitch)
}

void DrawBGMode7Background16PrioNew (uint16 *Screen, int bg, int prio,int pitch)
{
	if ((GFX.Mode7PriorityMask >> 7)!=prio) { return; } else {
    	DrawBGMode7Background16New (Screen, bg,pitch);
 	}
}
