...
void ProcessDebugCommand (char *Line)
{
	...
	for (int i = 0; i < 0x40; i += 2)
	{
	    if (i % 16 == 0)
		printf ("\n%04x ", 0xffc0 + i);
	    printf ("%04x ", (APUuncached->ExtraRAM) [i]);
	}
	printf ("\n");
    }
    if (Line[0] == 's')
    ...
    if (*Line == 'A')
    {
	(APUuncached->Flags) ^= TRACE_FLAG;
	extern FILE *apu_trace;
	if ((APUuncached->Flags) & TRACE_FLAG)
	{
#ifdef SPCTOOL
	    printf ("ENABLED\n");
	...
	printf ("APU tracing %s\n", (APUuncached->Flags) & TRACE_FLAG ? "enabled" :
		"disabled");
    }
    if (*Line == 'B')
    {
    ...
    if (*Line == 'C')
    {
	printf ("SPC700 sample addresses at 0x%04x:\n", (APUuncached->DSP) [APU_DIR] << 8);
	for (int i = 0; i < 256; i++)
	{
	    uint8 *dir = (IAPUuncached->RAM) + 
		    ((((APUuncached->DSP) [APU_DIR] << 8) +
		      i * 4) & 0xffff);
	...
	for (int l = 0; l < Count; l += 16)
	{
	    printf ("%04X ", Address);
	    for (int i = 0; i < 16; i++)
		printf ("%02X ", (IAPUuncached->RAM) [Address++]);
	    printf ("\n");
	}
	*Line = 0;
    }
	...
	printf ("APU in-ports: %02X %02X %02X %02X\n",
		(IAPUuncached->RAM) [0xF4], (IAPUuncached->RAM) [0xF5], (IAPUuncached->RAM) [0xF6], (IAPUuncached->RAM) [0xF7]);
#ifdef SPCTOOL
	printf ("APU out-ports: %02X %02X %02X %02X\n",
		_SPCOutP [0], _SPCOutP [1], _SPCOutP [2], _SPCOutP [3]);
#else
	printf ("APU out-ports: %02X %02X %02X %02X\n",
		(APUuncached->OutPorts) [0], (APUuncached->OutPorts) [1], (APUuncached->OutPorts) [2], (APUuncached->OutPorts) [3]);
#endif
	printf ("ROM/RAM switch: %s\n", ((IAPUuncached->RAM) [0xf1] & 0x80) ? "ROM" : "RAM");
	for (int i = 0; i < 3; i++)
	    if ((APUuncached->TimerEnabled) [i])
		printf ("Timer%d enabled, Value: 0x%03X, 4-bit: 0x%02X, Target: 0x%03X\n",
			i, (APUuncached->Timer) [i], (IAPUuncached->RAM) [0xfd + i], (APUuncached->TimerTarget) [i]);
    }
    if (*Line == 'P')
    {
