...
void pspDrawTile16 (uint32 Tile, s32 x,s32 y, uint32 StartLine,uint32 LineCount) {		
  ...
  if (BG.DirectColourMode) col=0;     
  else col = (((Tile >> 10) & BG.PaletteMask) << BG.PaletteShift) + BG.StartPalette;
...
void pspDrawClippedTile16 (uint32 Tile, s32 x,s32 y,uint32 StartPixel, uint32 Width,uint32 StartLine, uint32 LineCount) {		
#ifdef DEBUG_TILE
return;	
#endif
	uint32 u,v,col,TileKey,TileNumber,TileAddr;		
...
void pspDrawHiResTile16 (uint32 Tile, s32 x,s32 y, uint32 StartLine,uint32 LineCount) {
#ifdef DEBUG_TILE
return;	
#endif
	uint32 u,v,col,TileKey,TileNumber,TileAddr;		
  ...
  if (vertices_ptr[current_bitshift]==vertices_end[current_bitshift]){
	...
	if (!(Tile & (V_FLIP | H_FLIP))) {//NO FLIP
		vertices_ptr[current_bitshift][0].u = u+hires_offset; vertices_ptr[current_bitshift][0].v = v+StartLine+hires_offsetV;
		vertices_ptr[current_bitshift][1].u = u+8-hires_offset2; vertices_ptr[current_bitshift][1].v = v+StartLine+LineCount-hires_offsetV2;
	}else if (!(Tile & V_FLIP)) {//HFLIP
		vertices_ptr[current_bitshift][1].u = u+hires_offset; vertices_ptr[current_bitshift][0].v = v+StartLine+hires_offsetV;
		vertices_ptr[current_bitshift][0].u = u+8-hires_offset2; vertices_ptr[current_bitshift][1].v = v+StartLine+LineCount-hires_offsetV2;
	}else if (Tile & H_FLIP) {//HFLIP & VFLIP						
		vertices_ptr[current_bitshift][1].u = u+hires_offset; vertices_ptr[current_bitshift][1].v = v+8-StartLine-LineCount+hires_offsetV;
		vertices_ptr[current_bitshift][0].u = u+8-hires_offset2; vertices_ptr[current_bitshift][0].v = v+8-StartLine-hires_offsetV2;
	}else {//VFLIP
		vertices_ptr[current_bitshift][0].u = u+hires_offset; vertices_ptr[current_bitshift][1].v = v+8-StartLine-LineCount+hires_offsetV;
		vertices_ptr[current_bitshift][1].u = u+8-hires_offset2; vertices_ptr[current_bitshift][0].v = v+8-StartLine-hires_offsetV2;
	}
	vertices_ptr[current_bitshift]+=2;				

}

void pspDrawHiResClippedTile16 (uint32 Tile, s32 x,s32 y,uint32 StartPixel, uint32 Width,uint32 StartLine, uint32 LineCount) {
#ifdef DEBUG_TILE
return;	
#endif
	uint32 u,v,col,TileKey,TileNumber,TileAddr;		
...
void pspDrawLargePixel16 (uint32 Tile, s32 x,s32 y,uint32 StartPixel, uint32 Pixels,uint32 StartLine, uint32 LineCount) {
#ifdef DEBUG_TILE
return;	
#endif
	uint32 u,v,col,TileKey,TileNumber,TileAddr;		
  ...
  if (_verticesCol_ptr==_verticesCol_end){
	...
	//_verticesCol_ptr[0].color=_verticesCol_ptr[1].color=GFX.ScreenColors[tile_texture[current_bitshift][(v<<9)|u]];
	_verticesCol_ptr+=2;
}
