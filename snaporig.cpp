...
static int ReadBlock (const char *key, void *block, int max_len, STREAM snap)
{
    ...
    if (rem)
    {
	char *junk = (char*)malloc(rem);//new char [rem];
	READ_STREAM (junk, rem, snap);
	free(junk);//delete junk;
    }
...
static int ReadOrigSnapshot (STREAM snap)
{
    ...
    if ((result = ReadBlock ("VRA:", VRAM, 0x10000, snap)) != SUCCESS)
	return (result);
    if ((result = ReadBlock ("RAM:", RAM, 0x20000, snap)) != SUCCESS)
	return (result);
	...
	if ((result = ReadBlock ("ARA:", (IAPUuncached->RAM), 0x10000, snap)) != SUCCESS)
	    return (result);
	if ((result = ReadBlock ("SOU:", &OrigSoundData,
				 sizeof (SOrigSoundData), snap)) != SUCCESS)
	    return (result);

	SoundDataPtr->master_volume_left = OrigSoundData.master_volume_left;
	SoundDataPtr->master_volume_right = OrigSoundData.master_volume_right;
	SoundDataPtr->echo_volume_left = OrigSoundData.echo_volume_left;
	SoundDataPtr->echo_volume_right = OrigSoundData.echo_volume_right; 
	SoundDataPtr->echo_enable = OrigSoundData.echo_enable;
	SoundDataPtr->echo_feedback = OrigSoundData.echo_feedback;
	SoundDataPtr->echo_ptr = OrigSoundData.echo_ptr;
	SoundDataPtr->echo_buffer_size = OrigSoundData.echo_buffer_size;
	SoundDataPtr->echo_write_enabled = OrigSoundData.echo_write_enabled;
	SoundDataPtr->echo_channel_enable = OrigSoundData.echo_channel_enable;
	SoundDataPtr->pitch_mod = OrigSoundData.pitch_mod;

	for (i = 0; i < 3; i++)
	    SoundDataPtr->dummy [i] = OrigSoundData.dummy [i];
	for (i = 0; i < NUM_CHANNELS; i++)
	{
	    SoundDataPtr->channels [i].state = OrigSoundData.channels [i].state;
	    SoundDataPtr->channels [i].type = OrigSoundData.channels [i].type;
	    SoundDataPtr->channels [i].volume_left = OrigSoundData.channels [i].volume_left;
	    SoundDataPtr->channels [i].volume_right = OrigSoundData.channels [i].volume_right;
	    SoundDataPtr->channels [i].hertz = OrigSoundData.channels [i].frequency;
	    SoundDataPtr->channels [i].count = OrigSoundData.channels [i].count;
	    SoundDataPtr->channels [i].loop = OrigSoundData.channels [i].loop;
	    SoundDataPtr->channels [i].envx = OrigSoundData.channels [i].envx;
	    SoundDataPtr->channels [i].left_vol_level = OrigSoundData.channels [i].left_vol_level;
	    SoundDataPtr->channels [i].right_vol_level = OrigSoundData.channels [i].right_vol_level;
	    SoundDataPtr->channels [i].envx_target = OrigSoundData.channels [i].envx_target;
	    SoundDataPtr->channels [i].env_error = OrigSoundData.channels [i].env_error;
	    SoundDataPtr->channels [i].erate = OrigSoundData.channels [i].erate;
	    SoundDataPtr->channels [i].direction = OrigSoundData.channels [i].direction;
	    SoundDataPtr->channels [i].attack_rate = OrigSoundData.channels [i].attack_rate;
	    SoundDataPtr->channels [i].decay_rate = OrigSoundData.channels [i].decay_rate;
	    SoundDataPtr->channels [i].sustain_rate = OrigSoundData.channels [i].sustain_rate;
	    SoundDataPtr->channels [i].release_rate = OrigSoundData.channels [i].release_rate;
	    SoundDataPtr->channels [i].sustain_level = OrigSoundData.channels [i].sustain_level;
	    SoundDataPtr->channels [i].sample = OrigSoundData.channels [i].sample;
	    for (j = 0; j < 16; j++)
		SoundDataPtr->channels [i].decoded [j] = OrigSoundData.channels [i].decoded [j];

	    for (j = 0; j < 2; j++)
		SoundDataPtr->channels [i].previous [j] = OrigSoundData.channels [i].previous [j];

	    SoundDataPtr->channels [i].sample_number = OrigSoundData.channels [i].sample_number;
	    SoundDataPtr->channels [i].last_block = OrigSoundData.channels [i].last_block;
	    SoundDataPtr->channels [i].needs_decode = OrigSoundData.channels [i].needs_decode;
	    SoundDataPtr->channels [i].block_pointer = OrigSoundData.channels [i].block_pointer;
	    SoundDataPtr->channels [i].sample_pointer = OrigSoundData.channels [i].sample_pointer;
	    SoundDataPtr->channels [i].mode = OrigSoundData.channels [i].mode;
	}
...
//#endif	    
	...
	(IAPUuncached->APUExecuting) = FALSE;
	S9xSetSoundMute (TRUE);
    }
