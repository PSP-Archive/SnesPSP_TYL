...
int S9xTraceAPU ()
{
    ...
    uint8 b = S9xAPUOPrint (buffer, (IAPUuncached->PC) - (IAPUuncached->RAM));
    if (apu_trace == NULL)
...
{
    char mnem [100];
    uint8 *p = (IAPUuncached->RAM) + Address;
    int mode = Modes [*p];
    ...
    {
    case 1:
	sprintf (buffer, "%04X %02X       ", p - (IAPUuncached->RAM), *p);
	break;
    case 2:
	sprintf (buffer, "%04X %02X %02X    ", p - (IAPUuncached->RAM), *p,
		 *(p + 1));
	break;
    case 3:
	sprintf (buffer, "%04X %02X %02X %02X ", p - (IAPUuncached->RAM), *p,
		 *(p + 1), *(p + 2));
	break;
    }
    ...
    case DPREL:
	sprintf (mnem, S9xMnemonics [*p], *(p + 1),
		(int) (p + 3 - (IAPUuncached->RAM)) + (signed char) *(p + 2));
	break;
    case ABSBIT:
    ...
    case REL:
	sprintf (mnem, S9xMnemonics [*p],
		(int) (p + 2 - (IAPUuncached->RAM)) + (signed char) *(p + 1));
	break;
    }

    sprintf (buffer, "%s %-20s A:%02X X:%02X Y:%02X S:%02X P:%c%c%c%c%c%c%c%c %03dl %04dl %04dl",
	     buffer, mnem,
	     (APURegistersUncached->YA).B.A, (APURegistersUncached->X), (APURegistersUncached->YA).B.Y,
	     (APURegistersUncached->S),
...
void S9xPrintAPUState ()
{
    ...
    printf ("Noise: %s, Frequency: %d, Pitch mod: %s\n", as_binary ((APUuncached->DSP) [APU_NON]),
	    NoiseFreq [(APUuncached->DSP) [APU_FLG] & 0x1f],
	...
	{
	    int freq = ch->hertz;
	    if ((APUuncached->DSP) [APU_NON] & (1 << J)) //ch->type == SOUND_NOISE)
	    {
		freq = NoiseFreq [(APUuncached->DSP) [APU_FLG] & 0x1f];
		printf ("noise, ");
	    }
	    else
		printf ("sample %d, ", (APUuncached->DSP) [APU_SRCN + J * 0x10]);
